...
void findValidWords(Grid<char>& board, Lexicon& lex, Grid<bool>& visited, int row, int col, string prefix, Set<string>& validWords) {
    if (!board.inBounds(row, col)) {
        return;
    } else if (visited.get(row, col)) {
        return;
    } else if (!lex.containsPrefix(prefix += board.get(row, col))) {
        return;
    } else {
        visited.set(row, col, true);
        if (prefix.length() > 3 && lex.contains(prefix)) {
            validWords.add(prefix);
        }

        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                findValidWords(board, lex, visited, row + i, col + j, prefix, validWords);
            }
        }
...
int scoreBoard(Grid<char>& board, Lexicon& lex) {
    Set<string> validWordsOnBoard;
    Grid<bool> visited(board.numRows(), board.numCols(), false);

    for (int row = 0; row < board.numRows(); row++) {
        for (int col = 0; col < board.numCols(); col++) {
            findValidWords(board, lex, visited, row, col, "", validWordsOnBoard);
        }
    }

    int score = 0;
    for (string word: validWordsOnBoard) {
        score += points(word);
    }
    return score;
}
...
STUDENT_TEST("Test lex, confirms empty string is a prefix of all the words") {
    Lexicon lex = sharedLexicon();
